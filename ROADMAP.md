# Pixel Studio — 全量开发计划 (Phase B → F)

当前版本：**v0.3.1** ｜ JS 558 行 + HTML 340 行 = 898 行 ｜ 工具数：10

---

## Phase B — 图层系统 (目标 v0.5)

**核心变更：从单层数据结构 → 多层渲染管线。这是整个编辑器架构的决定性升级。**

### B1. 数据结构重构

`js
// 当前 (Phase A):
let peData = string[][]   // 单个二维数组
...
let peLayers = [
  {
    ...
    locked: false,
    opacity: 1.0,
    blendMode: 'normal',  // normal | multiply | screen | overlay | darken | lighten | difference
    data: string[][]        // 每层独立像素数据
  }
]
let peActiveLayer = 0      // 当前编辑的图层索引
`

**迁移策略：**
1. 初始化时创建 1 个默认图层，内容等于当前 peData
2. 所有绘图函数 (peSet, peSetBrush, peFloodFill, peDrawLine 等) 改为操作 peLayers[peActiveLayer].data
3. 渲染函数从 peRender() → peRenderComposite()，按顺序合并所有可见图层
4. 撤销/重做改为保存全量快照 JSON.parse(JSON.stringify(peLayers))

### B2. 混合模式实现

每个 Blend Mode 本质是一个像素级函数 lend(base, overlay) → result：

| 模式 | 数学 | 效果 |
|------|------|------|
| Normal | overlay | 叠加层直接覆盖 |
| Multiply | base * overlay / 255 | 变暗，适合阴影 |
| Screen | 255 - (255-base)*(255-overlay)/255 | 变亮，适合高光 |
| Overlay | base<128 ? 2*b*o/255 : 255-2*(255-b)*(255-o)/255 | 增强对比 |
| Darken | min(base, overlay) | 只保留更暗 |
| Lighten | max(base, overlay) | 只保留更亮 |
| Difference | abs(base - overlay) | 反转颜色 |

渲染管线伪代码：
`
for each layer (bottom to top):
  if not layer.visible: skip
  render layer.data to offscreen canvas
  apply blendMode with existing composite canvas
  apply opacity
...
**UI 元素清单：**
- 图层列表容器：.pe-layer-panel
- 每层缩略图 + 名称（双击重命名）
- 可见性切换按钮（👁 眼图标，点击切换 layer.visible）
- 锁定切换按钮（🔒 锁图标，点切换 layer.locked）
- 不透明度滑条（0-100%）
- 激活指示器（高亮边框 + 铅笔图标）
- 拖拽排序
- 操作按钮行：Add / Delete / Duplicate / Merge Down
- 每层缩略图用小 canvas 实时渲染层内容

### B4. 工具适配

| 工具 | 图层适配 |
|------|---------|
| Pencil / Eraser | 写入 peLayers[active].data |
| Fill | 操作当前激活层 |
| Line / Rect / Circle | 同上 |
| Spray | 同上 |
| Select | 视觉选区不变；Cut 清除激活层内的选区像素 |
| Replace | 只替换激活层内颜色 |
| Eyedropper | 读取 **复合渲染结果** 而不是单层 |
| Fill | 只填充激活层 |

### B5. 导出适配

- 默认导出：复合渲染（所有可见层合并）
- 新增导出选项：Current Layer Only / All Visible Layers
- 新增导出选项：Export Each Layer Separately（批量下载 zip，或逐张 PNG）

### B6. 撤销/重做升级

`js
// 当前: 保存 peData 快照
// 升级后: 保存完整 peLayers 快照
peHistory = []  // 每个条目 = JSON.parse(JSON.stringify(peLayers))
peHistoryIdx = -1
`

**优化：** 超过 50 步时丢弃最老的。每个图层操作（添加/删除/排序）也存历史。
...
// 激活层索引
__debug.getActiveLayer()  // -> 0

// 每层像素数据
__debug.getLayerData(0)  // -> string[][]

// 复合渲染检查
__debug.getCompositePixel(5, 5)  // -> '#ff0040' (混合后的颜色)

// 混合模式验证
// 在 Layer 1 画 #ff0000，Layer 2 画 #0000ff 并设 blendMode='screen'
__debug.getCompositePixel(5, 5)  // -> 预期接近 '#ff00ff'
`

### B8. HTML 新增元素

`html
<!-- 图层面板 (sidebar 替代旧 palette 区域) -->
<div class="pe-layer-panel">
  <div class="pe-layer-header">
    <h3>Layers</h3>
    <button id="layerAdd">+</button>
    <button id="layerDel">×</button>
  </div>
  <div class="pe-layer-list" id="layerList">
    <!-- 动态渲染 -->
  </div>
  <div class="pe-layer-actions">
    <button id="layerMerge">Merge Down</button>
    <button id="layerDup">Duplicate</button>
  </div>
</div>
`
...
- .pe-layer-panel — 图层面板容器
- .pe-layer-item — 单层条目（flex 行）
- .pe-layer-item.active — 激活状态高亮
- .pe-layer-thumb — 缩略图小 canvas
- .pe-layer-vis, .pe-layer-lock — 可见性/锁定图标按钮
- .pe-layer-opacity — 不透明度滑条
- 缩略图大小：48×48 或 56×56 px
